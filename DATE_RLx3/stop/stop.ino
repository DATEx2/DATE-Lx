...
const int int_semn_st = A1;  //intrare semnalizare stanga
const int int_semn_dr = A0;  //intrare semnalizare dreapta


//iesiri
const int out_poz_st = 10;
const int out_poz_dr = 9;
const int out_stop = 8;
const int out_st = 7;
const int out_dr = 6;
//altele
...
void loop() {
  ...
  if (stateFrana >= 200 && previousstateFrana <= 200) {
    franaa = 1;
  ...
  //pozitii
  if (analogRead(int_poz) >= 200) {
           if (analogRead(int_semn_st) >= 200) {
                digitalWrite(out_poz_st, ledState_st);}
             else { digitalWrite(out_poz_st, HIGH);}
           if (analogRead(int_semn_dr) >= 200) {
  ...
  switch(lastBlinkState%2) {
    ...
    case 1://blink off
      if (now > lastBlink + blinkForMs) 
      {
        ...
        int brakeState = completed ? (analogRead(int_stop) < 200 ? LOW : HIGH) : LOW;
        int pozitionSate = completed ? (analogRead(int_poz) < 200 ? LOW : HIGH) : LOW;
