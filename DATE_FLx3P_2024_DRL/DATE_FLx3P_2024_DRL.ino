...
int oLedRED = 11; // Output RED led button small circle
...
int oPoliceLights = 12; // Output on triple tap police lights / red+blue / red + red / blue + blue
int oTurnLeft = 7;//1102; // Output separate fluid left turn signal front side
int oTurnRight = 9;//1104; // Output separate fluid right turn signal front side
int oPositionLeft = 8; // Output separate fluid left turn signal rear side "position light"
int oPositionRight = 10; // Output separate fluid right turn signal rear side "position light"
...
enum HeadLightState { ManualOff, AutoOffExceptDRL, AutoOff, ManualOn, AutoOn };
...
unsigned long ShutDownAfterDisplayInSecondsAfterDisplayTurnsOff = 10; //should be 10s. auto off when the display is turned off
unsigned long TurnOffAfterSecondsEvenWhenDisplayIsStillOnExceptDRLAndPolice = 1800; //auto off when no manual comands (button or turn signals), except DRL and Police lights
unsigned long TurnOffAfterSecondsEvenWhenDisplayIsStillOnAll = 3600; //auto off when no manual comands (button or turn signals)
...
bool checkAutoOff() {
  ...
  if ((now >= sinceLastCommand + (TurnOffAfterSecondsEvenWhenDisplayIsStillOnExceptDRLAndPolice * 1000))
    && (headLightState == ManualOn || headLightState == AutoOn)) {    
    saveState();
    headLightState = AutoOffExceptDRL; //turn off everything except police light and except DRL
    currentState = (State)(currentState & ~(LowBeam | HighBeam | YRL | SecondaryLight));//turn off LowBeam, HighBeam, DRL, YRL, SecondaryLight flag
    if ((currentState & Turn3x) != 0) {
  ...
  if ((now >= sinceLastCommand + (TurnOffAfterSecondsEvenWhenDisplayIsStillOnAll * 1000))
    && (headLightState == ManualOn || headLightState == AutoOn || headLightState == AutoOffExceptDRL)) {              
   headLightState = AutoOff; //auto off
   if ((currentState != (State)(currentState & ~(LowBeam | HighBeam | YRL | SecondaryLight | DRL))) 
      || (currentPolice != (Police)(currentPolice & ~(PoliceLights)))) { //turn off LowBeam, HighBeam, DRL, YRL, SecondaryLight flag
     currentState = (State)(currentState & ~(LowBeam | HighBeam | YRL | SecondaryLight | DRL));//turn off LowBeam, HighBeam, DRL, YRL, SecondaryLight flag
     if ((currentState & Turn3x) != 0) {
        shouldStillBlinkLeftTurnSignal = 2;
        shouldStillBlinkRightTurnSignal = 2;
      }
      ...
      digitalWrite(oPoliceLights, (currentPolice & PoliceLights) == PoliceLights ? HIGH : LOW);
...
bool autoTurnOff(void *argument /* optional argument given to in/at/every */) {
  ...
  if (headLightState == AutoOn || headLightState == ManualOn) {
    saveState();
    headLightState = ManualOff; //manual off
    currentState = (State)(currentState & ~(LowBeam | HighBeam | YRL | SecondaryLight));//turn off LowBeam, HighBeam, DRL, YRL, SecondaryLight flag   
    if ((currentState & Turn3x) != 0) {
    ...
    //turn off everything//digitalWritePins(LOW, oHighBeam, oLowBeam, oYRL, oDRL, oSecondaryLight, oPoliceLights, oTurnLeft, oTurnRight, oPositionLeft, oPositionRight, oLoudHorn);
    sinceLastCommand = millis() - (TurnOffAfterSecondsEvenWhenDisplayIsStillOnExceptDRLAndPolice * 1000);
    applyState();
    headLightState = AutoOffExceptDRL; //turn off everything except police light and except DRL
  }
}
