...
int iDisplayOn = A6; // Input "display on" (brake)
...
void updateLedLight(){
  ...
  if (turnLeft || turnRight) { }
      ...
      digitalWriteStateToPins(oLedRED, HIGH, oLedGREEN, LOW, oLedBLUE, LOW, oYRL, HIGH, oPositionFront, HIGH, oPositionFront, HIGH);
...
void applyState(){
  ...
  if (!isOn && shouldStillBlinkLeftTurnSignal <= 0 && shouldStillBlinkRightTurnSignal <= 0 && (!(turnLeft || turnRight)) ) {
     digitalWritePins(LOW, oHighBeam, oLowBeam, oYRL, oDRL, oCharger, oTurnLeft, oTurnRight, oPositionFront, oPositionFront, oLoudHorn);
  }
  else {
    ...
    if (turnLeft || turnRight || positionTurnLeftShouldBeOff || positionTurnRightShouldBeOff) {
       digitalWriteStateToPins(oYRL, turnLeft || turnRight ? HIGH : LOW, 
                               ...
                               oPositionFront, positionTurnLeftShouldBeOff && positionTurnRightShouldBeOff ? LOW : (turnRight || positionShouldBeOn || turnLeft || positionShouldBeOn ? HIGH : LOW),
