...
int oCharger = 6; // Output phone charger
int oTurnLeft = 9;//1102; // Output separate fluid left turn signal front side
int oTurnRight = 7;//1104; // Output separate fluid left turn signal front side
...
int iTurnRight = A7; // Input right turn signal
int iOriginalLightOn = A3; // Input original light
int iHorn = A1; // Input horn
int iDisplayOn = A3; // Input "display on"
int iBrake = A6; // Input "display on" (brake)
...
unsigned long TurnOffAfterMilisecondsEvenWhenDisplayIsStillOn = 300000; //auto off when no manual comands (button or turn signals)
...
bool blinkOn(void *argument /* optional argument given to in/at/every */) {
  ...
  bool shouldTurnLeft = analogRead(iTurnLeft) > 100, shouldTurnRight = analogRead(iTurnRight) > 100;
  if (!prevTurnLeftState && shouldTurnLeft
      || !prevTurnRightState && shouldTurnRight) {
    sinceLastCommand = millis();
  }
  if (shouldTurnRight && !turnOffTimeout) {
    shouldStillBlinkLeftTurnSignal = 0;
  }
  if (shouldTurnLeft && !turnOffTimeout) {
    shouldStillBlinkRightTurnSignal = 0;
  }

  if ((currentState & Turn3x) != 0) {
    if (!shouldTurnLeft && prevTurnLeftState && !shouldTurnRight) {
      shouldStillBlinkLeftTurnSignal = 3;
    }
    if (!shouldTurnRight && prevTurnRightState && !shouldTurnLeft) {
      shouldStillBlinkRightTurnSignal = 3;
    }
  }
  //Serial.printf("shouldTurnLeft %d, shouldStillBlinkLeftTurnSignal %d\n", shouldTurnLeft, shouldStillBlinkLeftTurnSignal);
  if (shouldTurnLeft && !turnOffTimeout || shouldStillBlinkLeftTurnSignal > 0 && shouldStillBlinkLeftTurnSignal-- > 0) {
    currentBlink = (Blink)(currentBlink | TurnLeft);
  }

  if (shouldTurnRight && !turnOffTimeout || shouldStillBlinkRightTurnSignal > 0 && shouldStillBlinkRightTurnSignal-- > 0) {
...
void digitalWritePins(int state, int p0 = 0, int p1 = 0, int p2 = 0, int p3 = 0, int p4 = 0, int p5 = 0, int p6 = 0, int p7 = 0, int p8 = 0, int p9 = 0, int p10 = 0, int p11 = 0, int p12 = 0) {
  ...
  for (int i = ARRAY_SIZE(pins); --i >= 0;) if (pins[i] != 0) {
      digitalWrite(pins[i], state);
    }
}
void digitalWriteStateToPins(int pins[]) {
  for (int i = 0; i < ARRAY_SIZE(pins); i += 2) if (pins[i] != 0) {
      digitalWrite(pins[i], pins[i + 1]);
    }
}
...
void updateLedLight() {
  ...
  if (turnLeft || turnRight) { }
    ...
    digitalWriteStateToPins(oLedRED, HIGH, oLedGREEN, LOW, oLedBLUE, LOW, oYRL, HIGH, oPositionFront, HIGH);
...
bool lastTimeBrake = false;
bool lastTimeTurnOffTimeout = false;
void applyState() {

  unsigned long now = millis();
  bool turnOffTimeout = now >= (sinceLastCommand + TurnOffAfterMilisecondsEvenWhenDisplayIsStillOn);
  bool brake = analogRead(iBrake) > 200;
  ...
  //  Serial.printf("sinceLastCommand %ul, ",// iBrake %d, isOn %d, currentBlink %d, shouldStillBlinkLeftTurnSignal %d, shouldStillBlinkRightTurnSignal %d, turnLeft %d, turnRight %d\n",
  //    sinceLastCommand);//,  brake, isOn, currentBlink, shouldStillBlinkLeftTurnSignal, shouldStillBlinkRightTurnSignal, turnLeft, turnRight);

  //  Serial.printf("now %ul, ", //iBrake %d, isOn %d, currentBlink %d, shouldStillBlinkLeftTurnSignal %d, shouldStillBlinkRightTurnSignal %d, turnLeft %d, turnRight %d\n",
  //    now);//,  brake, isOn, currentBlink, shouldStillBlinkLeftTurnSignal, shouldStillBlinkRightTurnSignal, turnLeft, turnRight);
  //  Serial.printf("brake %d, ",//isOn %d, currentBlink %d, shouldStillBlinkLeftTurnSignal %d, shouldStillBlinkRightTurnSignal %d, turnLeft %d, turnRight %d\n",
  //    brake);//, isOn, currentBlink, shouldStillBlinkLeftTurnSignal, shouldStillBlinkRightTurnSignal, turnLeft, turnRight);
  //  Serial.printf("isOn %d, currentBlink %d, shouldBlinkLeft %d, shouldBlinkRight %d, turnLeft %d, turnRight %d\n",
  //    isOn, currentBlink, shouldStillBlinkLeftTurnSignal, shouldStillBlinkRightTurnSignal, turnLeft, turnRight);
  if (!lastTimeBrake && brake) {
    if (headLightState == ManualOff) {
      sinceLastCommand = millis();
    } else {
      restoreState();
    }
  }
  lastTimeBrake = brake;//to prevent when the display turns off, as it reports the break is on

  if (!lastTimeTurnOffTimeout && turnOffTimeout) {
    shouldStillBlinkLeftTurnSignal = shouldStillBlinkRightTurnSignal = 2;
  }
  ...
  if (turnOffTimeout && shouldStillBlinkLeftTurnSignal <= 0 && shouldStillBlinkRightTurnSignal <= 0) {
    ...
    digitalWriteStateToPins(oRLx1, brake || turnLeft || turnRight || isOn ? HIGH : LOW,
    ...
    if (turnLeft || turnRight || positionTurnLeftShouldBeOff || positionTurnRightShouldBeOff) {
      digitalWriteStateToPins(oYRL, turnLeft || turnRight ? HIGH : LOW,
                              ...
                              oPositionFront, positionTurnLeftShouldBeOff ? LOW : (turnRight || positionShouldBeOn || turnLeft || positionShouldBeOn ? LOW : LOW),
                              oTurnLeft, turnLeft ? HIGH : LOW,
                              //soPositionFront, positionTurnRightShouldBeOff ? LOW : (turnRight || positionShouldBeOn ? HIGH : LOW),
      ...
      digitalWriteStateToPins(oDRL, (currentState & DRL) != 0 ? HIGH : LOW,
                              ...
                              oPositionFront, positionShouldBeOn ? HIGH : LOW,
...
void longTap() {
  ...
  applyState();
  sinceLastCommand = millis() - TurnOffAfterMilisecondsEvenWhenDisplayIsStillOn;//turn the stop light RLx1 too
}
...
void setup() {
  ...
  pinMode(oPositionFront, OUTPUT); digitalWrite(oPositionFront, LOW);
...
void checkHornState() {
  if (analogRead(iHorn) > 100 && (currentState & DisableHorn) == 0) {
